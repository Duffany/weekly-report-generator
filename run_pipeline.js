/**
 * Node.js runner — mirrors the fixed app.js pipeline exactly.
 * Usage: node run_pipeline.js
 * Reads from "inputs and outputs S10/" and writes pipeline_output.xlsx there.
 */
...
const DIR = path.join(__dirname, 'inputs and outputs S10');

// ── Category mapping (exact copy from app.js) ─────────────────
const CATEGORIE_MAP = {
...
function normalizeGtin(raw) {
  ...
  // Remove trailing .0 and strip leading zeros (normalize both sides for matching)
...
async function main() {
  ...
  // Repartition SKU 1P — from manual file's sheet (template equivalent)
  const repSkuMap = new Map();
  try {
    const tplWb = readWb(path.join(DIR, 'Weekly Report Stock S10 + FULL FFM.xlsx'));
    const { rows: rskuRows } = sheetToArrays(tplWb, 'Repartition sku 1P');
    for (const row of rskuRows) {
      const gtinRaw = row[0]; const type = String(row[1] || '').trim();
      const ownerCode = String(row[4] || '').trim();
      if (!gtinRaw || !type) continue;
      const gtin = typeof gtinRaw === 'number' ? String(Math.round(gtinRaw)) : String(gtinRaw).trim();
  ...
  // Stock
  const stockWb = readWb(path.join(DIR, 'stock.xlsx'));
  ...
  // Conso
  const consoWb = readWb(path.join(DIR, 'Conso 1603.xlsx'));
  ...
  // L3M
  const l3mWb = readWb(path.join(DIR, 'Perf L3M.xlsx'));
  ...
  const LC = {
    ...
    marge: colIdx(lH,'marge l3M','Marge L3M','marge_l3m','Marge','marge'),
  ...
  // Jumia
  const jumiaWb = readWb(path.join(DIR, 'IP jumia 1503.xlsx'));
  const { headers: jH, rows: jumiaRows } = sheetToArrays(jumiaWb);
  const JC = {
    pid:  colIdx(jH,'sku','SKU','ProductId'),
    px:   colIdx(jH,'prix_jumia','Prix_Jumia'),
  ...
  for (const sRow of stockRows) {
    ...
    if (!pid) continue; // skip metadata rows
  ...
  const outPath = path.join(DIR, 'pipeline_output_S10.xlsx');
