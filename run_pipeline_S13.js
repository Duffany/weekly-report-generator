/**
 * Node.js runner for S13 — adapted from run_pipeline.js.
 * Input folder : "S12-S13 validation run/"
 * RepSKU source: Weekly Report Stock S12 + FULL FFM 3.xlsx
 * Output       : S12-S13 validation run/pipeline_output_S13.xlsx
 *
 * Key differences vs S10:
 *  - RepSku sheet col layout: col0=SKU, col1=GTIN, col2=Type, col5=Owner
 *  - L3M marge column: "Gross Margin" (col 42)
 *  - Jumia price column: "prix" (not "prix_jumia")
 */
...
const DIR = path.join(__dirname, 'S12-S13 validation run');
...
function buildVueGlobale(ws, totalRetailRows) {
  ...
  sectionStarts.forEach((col, i) => {
    const endColFull = i < 4
      ? String.fromCharCode(col.charCodeAt(0) + 5)
      : (i === 4 ? 'AE' : 'AK');
  ...
  VG_SECTIONS.forEach((sec, i) => {
    ...
    styleCell(ws.getCell(`${col}1`), sec.name, S.hFill, S.hFont, S.hAlign, S.hBorderRow1);
  });
  sectionStarts.forEach(col => {
    const c0 = colLetterToNum(col);
    ['B1','B2','Total'].forEach((lbl, j) => {
      styleCell(ws.getCell(`${numToColLetter(c0 + j*2)}2`), lbl, S.hFill, S.hFont, S.hAlign, S.hBorderRow2);
    });
  });
  ...
  sectionStarts.forEach(col => {
    ...
    for (let j = 0; j < 6; j++) {
      ...
      styleCell(ws.getCell(`${numToColLetter(c0+j)}3`), lbl, S.hFill, S.hFont, S.hAlign, S.hBorderRow3);
    }
  });

  const N = totalRetailRows;
  CATEGORIES_ORDER.forEach((cat, idx) => {
    const r = idx + 4;
    const row = ws.getRow(r);
    row.height = 15;
    const catCell = row.getCell(1);
    catCell.value = cat; catCell.alignment = S.dAlign; catCell.border = S.dBorder;
    catCell.font = { size: 11, name: 'Calibri' };

    sectionStarts.forEach((col, si) => {
      const sec = VG_SECTIONS[si];
      const c0  = colLetterToNum(col);
      ...
      [[skuF,stkF],[skuF2,stkF2],[skuFT,stkFT]].forEach(([sf,tf], j) => {
        const cSku = row.getCell(c0 + j*2);
        const cStk = row.getCell(c0 + j*2 + 1);
  ...
  const tRow = ws.getRow(totalR);
  tRow.height = 15;
  const tCell = tRow.getCell(1);
  tCell.value = 'Total général'; tCell.alignment = S.dAlign; tCell.border = S.dBorder;
  tCell.font = { bold: true, size: 11, name: 'Calibri' };

  sectionStarts.forEach((col) => {
    const c0 = colLetterToNum(col);
    for (let j = 0; j < 6; j++) {
      const startRow = 4;
      const endRow   = startRow + CATEGORIES_ORDER.length - 1;
      const colL     = numToColLetter(c0 + j);
      const c = tRow.getCell(c0 + j);
      c.value = { formula: `=SUM(${colL}${startRow}:${colL}${endRow})` };
      c.alignment = S.dAlign; c.border = S.dBorder;
      c.font = { bold: true, size: 11, name: 'Calibri' }; c.numFmt = '#,##0';
    }
  });
}

// ── Report Retail builder ──────────────────────────────────────
const RR_HEADERS = [
...
function buildReportRetail(ws, retailRows) {
  ...
  RR_HEADERS.forEach((h, i) => {
    ...
    c.value = h; c.font = { bold: true, size: 11, name: 'Calibri' };
...
async function main() {
  ...
  // ── RepSKU from S12 weekly report ──────────────────────────
  // S12 sheet layout: col0=SKU, col1=GTIN, col2=Type, col3=(empty), col4=CatRevue, col5=Owner
  const repSkuMap = new Map();
  try {
    const tplWb = readWb(path.join(DIR, 'Weekly Report Stock S12 + FULL FFM 3.xlsx'));
    const { rows: rskuRows } = sheetToArrays(tplWb, 'Repartition sku 1P');
    for (const row of rskuRows) {
      const skuRaw  = String(row[0] || '').trim();   // col0 = SKU (ProductId)
      const gtinRaw = row[1];                         // col1 = GTIN
      const type    = String(row[2] || '').trim();    // col2 = Type
      const ownerCode = String(row[5] || '').trim();  // col5 = Owner code
      if (!type) continue;
      // Index by GTIN
      if (gtinRaw !== '' && gtinRaw != null) {
        const gtin = typeof gtinRaw === 'number'
          ? String(Math.round(gtinRaw))
          : String(gtinRaw).trim().replace(/^0+(\d)/, '$1');
        if (gtin) repSkuMap.set(gtin, { type, owner: ownerCode });
      }
      // Index by SKU/ProductId as fallback
      if (skuRaw && !repSkuMap.has(skuRaw)) {
        repSkuMap.set(skuRaw, { type, owner: ownerCode });
      }
    }
    console.log(`RepSKU map: ${repSkuMap.size} entries`);
  } catch(e) { console.warn('RepSKU load failed:', e.message); }

  // ── Stock ──────────────────────────────────────────────────
  const stockWb = readWb(path.join(DIR, 'data - 2026-04-06T222153.798.xlsx'));
  ...
  // ── Conso ──────────────────────────────────────────────────
  const consoWb = readWb(path.join(DIR, 'conso 0604 1.xlsx'));
  ...
  // ── L3M ────────────────────────────────────────────────────
  // S13 L3M file: marge is "Gross Margin" (col42), no Cout unitaire column
  const l3mWb = readWb(path.join(DIR, 'data - 2026-04-06T223124.774.xlsx'));
  ...
  for (const row of l3mRows) {
    ...
    if (l3mMap.has(k)) {
      const e=l3mMap.get(k); e.pv+=pv; e.is+=is_; e.gmv+=gmv;
    } else {
  ...
  // ── Jumia ──────────────────────────────────────────────────
  // S13 Jumia file: price col is "prix" (not "prix_jumia"), sku at col3
  const jumiaWb = readWb(path.join(DIR, 'data - 2026-04-06T222858.787.xlsx'));
  ...
  for (const sRow of stockRows) {
    ...
    const margePct= lDat ? lDat.marge         : 0;
  ...
  const outPath = path.join(DIR, 'pipeline_output_S13.xlsx');
